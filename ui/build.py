...
r=root/'review/build.py';t=re.sub(r"gomoku-v12\.1\.0-review-1\.0\.0(?:-table-[A-Za-z0-9.\-]+)?",'gomoku-v12.1.0-review-1.0.0-table-1.0.2-audit',r.read_text())
# Avoid repeated suffixes if the source is already migrated.
t=t.replace('-table-1.0.0-table-1.0.0','-table-1.0.0');r.write_text(t)
sw=root/'sw.js';t=sw.read_text();t=re.sub(r"const CACHE_NAME = '[^']+';","const CACHE_NAME = 'gomoku-v12.1.0-review-1.0.0-table-1.0.2-audit';",t,count=1);sw.write_text(t)
print('Embedded Tournament Table 1.0.2; full-app regression repair.')
