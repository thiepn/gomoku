...
with sync_playwright() as p:
    ...
    pg.locator('#point-112').tap();check('touch selects before committing',pg.evaluate('GomokuStudio.diagnostics().moves')==0)
    check('touch confirmation remains reachable',pg.locator('#placeBtn').is_visible());pg.locator('#placeBtn').tap();pg.wait_for_function('GomokuStudio.diagnostics().moves>=1');check('touch confirmation places the intended stone',pg.evaluate('GomokuStudio.exportGame().moves[0].i')==112)
