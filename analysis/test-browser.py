...
with sync_playwright() as p:
 ...
 try:
  ...
  check('review exposes Analysis 2.0 instead of an unrelated new mode',pg.locator('#grTitle').inner_text()=='Game review' and '2.0' in pg.locator('.gr-kicker').first.inner_text())
  ...
  pg.evaluate('GomokuReview.select(7)');pg.locator('#a2Preset').select_option('deep');pg.locator('#grDeeper').click();pg.wait_for_function('!GomokuReview.state().interacting',timeout=25000)
  ...
  pg.locator('#a2Overlays').uncheck();check('optional board markers turn off',pg.locator('#grBoard .gr-suggested').count()==0 and pg.locator('#grBoard .gr-threat').count()==0);pg.locator('#a2Overlays').check()
  pg.locator('#a2Preset').select_option('quick');pg.locator('#grDeeper').click();check('lower budget cannot overwrite deeper saved evidence',pg.evaluate('GomokuReview.state().results[6].budget')==2400)
  pg.locator('#a2Preset').select_option('maximum');pg.locator('#grDeeper').click();pg.wait_for_timeout(40);pg.locator('#grPause').click();check('Maximum search is interruptible',not pg.evaluate('GomokuAnalysisRuntime.stats().busy'))
  pg.locator('#a2Preset').select_option('deep');pg.evaluate('GomokuReview.select(8)');pg.locator('#a2BestProof').click();pg.wait_for_function('GomokuReview.state().mode==="proof"',timeout=20000);check('winning alternative also has a proof entry',pg.locator('#a2ProofTitle').inner_text()=='Why the best move wins');pg.locator('#grReturn').click()
